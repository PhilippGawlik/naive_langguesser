...
pub fn get_probability(nominator: i32, denominator: i32) -> Result<f32, UtilError> {
    if denominator > 0 {
        ...
        Err(UtilError::new("get_probability: Division by zero or negative number!"))
    }
}


pub fn get_probalities(counts: &HashMap<(char, char, char), i32>) -> Result<HashMap<(char, char, char), f32>, UtilError> {
            ...
            if path.is_dir() {
                ...
                let path_str = String::from(
                    path
                        ...
                        .ok_or(UtilError::new("get_model_paths: Can't convert path to string."))?
                );
                // only build regex once
                lazy_static! {
                        static ref is_model: Regex = Regex::new(r".*\.model")
                            .expect("get_model_path: Can't initialise regex.");
                    }
